...
import com.wanliang.small.entity.ProductCategory;
...
public class ProductDaoImpl extends BaseDaoImpl<Product, Long> implements ProductDao {
	...
	public List<Product> findList(ProductCategory productCategory, Brand brand, Promotion promotion, List<Tag> tags, Map<Attribute, String> attributeValue, BigDecimal startPrice, BigDecimal endPrice, Boolean isMarketable, Boolean isList, Boolean isTop, Boolean isGift, Boolean isOutOfStock, Boolean isStockAlert, OrderType orderType, Integer count, List<Filter> filters, List<com.wanliang.small.Order> orders) {
		CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
		CriteriaQuery<Product> criteriaQuery = criteriaBuilder.createQuery(Product.class);
		Root<Product> root = criteriaQuery.from(Product.class);
		criteriaQuery.select(root);
		...
		if (orderType == OrderType.priceAsc) {
			orders.add(com.wanliang.small.Order.asc("price"));
			orders.add(com.wanliang.small.Order.desc("createDate"));
		} else if (orderType == OrderType.priceDesc) {
			orders.add(com.wanliang.small.Order.desc("price"));
			orders.add(com.wanliang.small.Order.desc("createDate"));
		} else if (orderType == OrderType.salesDesc) {
			orders.add(com.wanliang.small.Order.desc("sales"));
			orders.add(com.wanliang.small.Order.desc("createDate"));
		} else if (orderType == OrderType.scoreDesc) {
			orders.add(com.wanliang.small.Order.desc("score"));
			orders.add(com.wanliang.small.Order.desc("createDate"));
		} else if (orderType == OrderType.dateDesc) {
			orders.add(com.wanliang.small.Order.desc("createDate"));
		} else {
			orders.add(com.wanliang.small.Order.desc("isTop"));
			orders.add(com.wanliang.small.Order.desc("modifyDate"));
		}
	...
	public List<Object[]> findSalesList(Date beginDate, Date endDate, Integer count) {
		...
		Join<Product, Order> order = orderItems.join("order");
	...
	public Page<Product> findPage(ProductCategory productCategory, Brand brand, Promotion promotion, List<Tag> tags, Map<Attribute, String> attributeValue, BigDecimal startPrice, BigDecimal endPrice, Boolean isMarketable, Boolean isList, Boolean isTop, Boolean isGift, Boolean isOutOfStock, Boolean isStockAlert, OrderType orderType, Pageable pageable) {
		...
		criteriaQuery.where(restrictions);
		List<com.wanliang.small.Order> orders = pageable.getOrders();
		if (orderType == OrderType.priceAsc) {
			orders.add(com.wanliang.small.Order.asc("price"));
			orders.add(com.wanliang.small.Order.desc("createDate"));
		} else if (orderType == OrderType.priceDesc) {
